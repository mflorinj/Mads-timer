...
app.http('timesheet', {
  ...
  handler: async (request) => {
    ...
    if(targetRead !== me && !admin) return { status: 403, jsonBody: { error: 'Ingen adgang til andres timer' } };
    ...
    try {
      if(request.method === 'GET'){
        if(year && week){
          try {
            ...
            return { jsonBody: item ? { year: item.year, week: item.week, rows: item.rows } : { year: Number(year), week: Number(week), rows: null } };
          } catch(e){ return { jsonBody: { year: Number(year), week: Number(week), rows: null } }; }
        }
        ...
        return { jsonBody: res.resources };
      }

      if(request.method === 'PUT'){
        let b = {};
        try { b = await request.json(); } catch(e){ b = {}; }
        if(b.year == null || b.week == null || !Array.isArray(b.rows)) return { status: 400, jsonBody: { error: 'year, week og rows kræves' } };
        const doc = {
      ...
      if(request.method === 'DELETE'){
        if(!year || !week) return { status: 400, jsonBody: { error: 'year og week kræves' } };
